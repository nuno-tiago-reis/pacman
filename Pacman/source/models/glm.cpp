...
int glmFindOrAddTexture(GLMmodel* model, char* name,mycallback *call)
{
	...
	if (call) 
	{
		int procent = (int)(((float)((float)model->numtextures*30/total_textures)/100)*(call->end-call->start)+call->start);
            ...
            case '\0':          /* vertex */
				...
				if (numvertices%200==0)
					if (call)
					{
						...
						int procent = (int)(((float)((float)numvertices*70/model->numvertices+30)/100)*(call->end-call->start)+call->start);
    ...
    for (i = 1; i <= model->numnormals; i++) {
        ...
        if(r == 0.0f) {
            ...
            if(y == 0.0f)
                ...
                theta = (GLfloat)asin(y / r) + (3.14159265f / 2.0f);
        }
